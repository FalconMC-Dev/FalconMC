...
use falcon_core::{ShutdownHandle, world};
...
pub(crate) fn start_server(shutdown_handle: ShutdownHandle) -> Result<()> {
    ...
    let config = FalconConfig::global();

    let mut world = None;
    if let Some(world_file_path) = config.world_file() {
        world = {
                let world_file = std::fs::read(world_file_path)
                ...
                Some(FalconWorld::try_from(data)?)
            };
        info!("Loaded world");   
    }
