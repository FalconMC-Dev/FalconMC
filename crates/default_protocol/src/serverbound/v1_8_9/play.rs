...
mod inner {
    ...
    impl PacketHandler for PlayerPositionPacket {
        fn handle_packet(self, connection: &mut dyn MinecraftConnection) -> TaskScheduleResult {
            if let Some(uuid) = connection.handler_state().player_uuid() {
                connection.server()
                    .player_update_pos_look(uuid, Some(self.x), Some(self.y), Some(self.z), None, None, Some(self.on_ground));
            }
            Ok(())
        }
    ...
    impl PacketHandler for PlayerLookPacket {
        fn handle_packet(self, connection: &mut dyn MinecraftConnection) -> TaskScheduleResult {
            ...
            connection.server()
                .player_update_pos_look(uuid, None, None, None, Some(self.yaw), Some(self.pitch), Some(self.on_ground));
            Ok(())
        }
    ...
    impl PacketHandler for PositionLookPacket {
        fn handle_packet(self, connection: &mut dyn MinecraftConnection) -> TaskScheduleResult {
            let uuid = connection.handler_state().player_uuid().expect("Something impossible happened");
            connection.server()
                .player_update_pos_look(uuid, Some(self.x), Some(self.y), Some(self.z), Some(self.yaw), Some(self.pitch), Some(self.on_ground));
            Ok(())
        }
