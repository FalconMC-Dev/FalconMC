...
pub fn into_block_data(
    raw_data: &RawBlockData,
    property_base: &mut EnumPropertyBase,
    ...
    for (name, value) in raw_base_state.properties().unwrap() {
        let found_property = properties
            .iter()
            .find(|x| x.name() == &avoid_type(name.clone()))
            .unwrap();
    ...
    if let Some(raw_properties) = raw_properties {
        for (raw_name, raw_values) in raw_properties.iter() {
            ...
            // case 2: integer range
                ...
                properties.push(BlockProperty::new(
                    property_name,
                    PropertyType::Int(
                        start..(raw_values.last().map(|x| x.parse::<u8>().unwrap()).unwrap() + 1),
                    ),
                ));
            // case 3: enum property
                ...
                match property_base.find_property(&property_name, raw_values) {
                    None => {
                        if default_version {
                            let property = property_base.add(EnumProperty::new(
                                raw_name.to_case(Case::Pascal),
                                raw_values.clone(),
                            ));
                            properties.push(BlockProperty::new(
                                property_name,
                            ...
                            match property_base.find_fields(raw_values) {
                                Some(second_chance) => {
                                    let enum_property = EnumProperty::with_order(
                                        String::from(second_chance.name()),
                                        raw_values.clone(),
                                        second_chance.fields_cloned(),
                                    );
                                    let block_property = BlockProperty::new(
                                        property_name,
                                        PropertyType::Enum(enum_property),
                                    );
                    ...
                    Some(base_property) => {
                        if default_version {
                            properties.push(BlockProperty::new(
                                property_name,
                                PropertyType::Enum(base_property.clone()),
                            ));
                        } else {
                            ...
                            if raw_values.len() < base_property.entry_count() {
                                properties.push(BlockProperty::new(
                                    property_name,
                                    PropertyType::Enum(EnumProperty::with_order(
                                        String::from(base_property.name()),
                                        raw_values.clone(),
                                        base_property.fields_cloned(),
                                    )),
                                ));
                            } else {
                                properties.push(BlockProperty::new(
