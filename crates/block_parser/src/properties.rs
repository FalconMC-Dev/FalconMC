...
impl PropertyType {
    pub fn from_raw(name: String, values: Vec<String>) -> Self {
        ...
        let list = PROPERTIES.lock().unwrap();
        if list.borrow().iter().find(|x| x.fields.eq(&property.fields) || x.name.eq(&property.name)).is_none() {
            list.borrow_mut().push(property.clone());
            return PropertyType::Enum((property, None));
        }
        let x = PropertyType::Enum((list.borrow().iter().find(|x| x.fields.eq(&property.fields) || x.name.eq(&property.name)).unwrap().clone(), None)); x
    }
    ...
    pub fn find(name: String, values: Vec<String>) -> Option<Self> {
        if values.first().unwrap() == "true" {
            ...
            return Some(PropertyType::Int(start..(values.last().unwrap().parse::<u8>().unwrap() + 1)));
        }
        let property = EnumProperty::new(name.to_case(Case::Pascal), values);
        let list = PROPERTIES.lock().unwrap();
        let list = list.borrow();
        if let Some(prop) = match list.iter().find(|x| x.fields.eq(&property.fields) || x.name.eq(&property.name)) {
            Some(res) => {
            ...
            None => list.iter().find(|x| property.fields.iter().all(|y| x.fields.contains(y))),
        } {
