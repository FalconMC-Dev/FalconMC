...
pub(crate) fn packet_structs_to_version_outgoing_list(packet_structs: &[PacketStruct]) -> HashMap<Ident, (LitStr, HashMap<i32, Vec<i32>>)> {
    let mut map = HashMap::new();
    for packet_struct in packet_structs {
        if let Some(name) = &packet_struct.outgoing {
            ...
            for (packet_id, version_list) in &packet_struct.versions {
                for version in version_list {
                    let (_, id_map) = map.entry(struct_name.clone()).or_insert_with(|| (name.clone(), HashMap::new()));
